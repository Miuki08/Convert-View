...
export default function ClientStatsCard({ title, value, iconName, trend }: StatsCardProps) {
      ...
      <div className="flex">
        ...
        <div className="flex items-center justify-center p-4">
          <div className={`w-12 h-12 rounded-full ${getColorClasses('bg')} flex items-center justify-center shadow-md`}>
            <IconComponent className={`w-6 h-6 ${getTextColorForBackground(getColorClasses('bg'))}`} />
          </div>
        </div>
      </div>
    </div>
